...
(function() {
    ...
    // Mỗi đường có 1 dot chạy
    const dots = cardIds.map((id, i) => ({
        id,
        color: colors[i],
        progress: Math.random(), // bắt đầu ngẫu nhiên
    ...
    function draw() {
        ...
        dots.forEach(dot => {
            ...
            ctx.strokeStyle = dot.color.replace(')', ', 0.15)').replace('rgb', 'rgba').replace('#', 'rgba(').replace('rgba(', 'rgba(') ;
            ctx.lineWidth = 1;
            ctx.setLineDash([4, 6]);

            // Parse hex color với opacity
